...
public:
    ...
    bool   contains(bromath::Vec3 p, int excludeTag = -1, float extraClearance = 0.0f) const;

    /// True if signed distance to the nearest surface is <= clearance.
    /// Equivalent to `distance(p, exclude) <= clearance`.
    bool   tooClose(bromath::Vec3 p, float clearance, int excludeTag = -1) const;

    /// Signed distance to the nearest obstacle surface. Negative inside,
    /// positive outside. Returns +infinity when the field is empty.
    float  distance(bromath::Vec3 p, int excludeTag = -1) const;

    struct Nearest {
        bromath::Vec3  point{};        // closest point on the surface
        bromath::Vec3  normal{};       // outward unit normal at that point
        float distance = 0.0f;// signed; negative inside
        int   tag      = -1;  // tag of the nearest obstacle, or -1 if empty
    };
    Nearest nearest(bromath::Vec3 p, int excludeTag = -1) const;

    /// True iff a sphere centered at `c` with `radius` overlaps any obstacle.
    bool   intersectsSphere(bromath::Vec3 c, float radius, int excludeTag = -1) const;
